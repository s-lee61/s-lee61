...
# Bring the diseases prediction model
...
# the SurveyMonkey invitation script
...
# Prediction button
    if st.button("Predict Lung Cancer"):
        ...
        user_data.replace({'NO': 0, 'YES': 1}, inplace=True)
...
# convert columns to numeric where necessary
        numeric_columns = ['AGE', 'SMOKING', 'YELLOW_FINGERS', 'ANXIETY', 'CHRONIC_DISEASE', 'FATIGUE', 'ALLERGY', 'ALCOHOL_CONSUMING', 'COUGHING', 'SHORTNESS_OF_BREATH', 'SWALLOWING_DIFFICULTY', 'CHEST_PAIN']
...
# after the prediction is done if the value in the list at index is 0 is 1 then the person is positive
...
if selected == 'Thyroid Disease Prediction':
    ...
    referral_source = st.selectbox("Referral Source", [0, 1, 2, 3, 4])
